...
class Builder (object):
	...
	def name_range (self, name):
		namefrom, nameto = list(map(lambda x: int(re.match ('.*(\d+)', x).group(1)), name.split('~')))
	...
	def merge_blocks_into_arrays (self):
		for groupname in self.peripheral_registers_arrayable:
			...
			for transform in transformations:
				...
				to_remove = list(filter (lambda x: x if re.match(transform, x.name) else None, self.peripheral_registers[groupname]))
				# print(to_remove)
